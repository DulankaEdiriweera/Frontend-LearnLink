...
const GoalCard = ({ goal, currentUser }) => {
  ...
  useEffect(() => {
    ...
    console.log("likedUsers", goal.likedUsers);
    console.log("currentUser", currentUser.email);
  }, [goal.likedUsers, currentUser]);
  ...
  const handleLike = async () => {
    try {
      ...
      const res = await axios.put(
        `http://localhost:8080/api/Goals/${goal.id}/like`,
        {},
        {
  ...
  const fetchLikedUsers = async () => {
    try {
      const res = await axios.get(
        `http://localhost:8080/api/Goals/${goal.id}/liked-users`
      );
    ...
    <div className="bg-white shadow-lg rounded-lg p-4 mb-4">
      ...
      <div className="flex items-center mb-2">
        <div className="font-semibold text-lg text-gray-800">
          {goal.user ? goal.user.username : "Unknown User"}
        </div>
      </div>
      ...
      {/* Goal Image or Video */}
      <div className="mt-4">
        {goal.imageUrl && goal.imageUrl.endsWith(".mp4") ? (
